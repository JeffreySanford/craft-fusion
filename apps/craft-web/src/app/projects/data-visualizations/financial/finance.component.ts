...
export class FinanceComponent implements OnInit, OnChanges {
  ...
  private getPhaseBackgroundColor(phaseType: Phase['type']): string {
    switch (phaseType) {
      case 'bull':
        return 'rgba(235, 251, 238, 0.7)'; // Light green background
      case 'bear':
        return 'rgba(253, 235, 236, 0.7)'; // Light red background
      case 'consolidation':
        return 'rgba(252, 248, 232, 0.7)'; // Light yellow/orange background
      default:
        return 'rgba(240, 240, 240, 0.7)'; // Light grey
    }
  }
  ...
  private renderStocksView(
    ...
    const tooltip = d3.select(this.chartContainer.nativeElement).append('div').attr('class', 'tooltip').style('opacity', 0).style('position', 'absolute').style('z-index', '1500'); // Higher z-index to prevent overlap
    ...
    stocks.forEach((stock: Stock, index: number) => {
      ...
      const addStockLabels = (data: unknown[], stockSymbol: string, lineIndex: number) => {
        ...
        const tempText = labelGroup.append('text').text(labelText).style('visibility', 'hidden'); // Invisible, just for measuring
  ...
  private getPhaseStrokeColor(phaseType: string): string {
    switch (phaseType) {
      ...
      case 'bear':
        return '#e76f51'; // Darker red
      case 'consolidation':
        return '#f4a261'; // Darker orange
      default:
        return '#6b705c'; // Darker grey
    }
  }
  ...
  private getPhaseLabelColor(phaseType: string): string {
    switch (phaseType) {
      case 'bull':
        return '#264653'; // Dark text on light background
      case 'bear':
        return '#264653'; // Dark text on light background
      case 'consolidation':
        return '#264653'; // Dark text on light background
      default:
        return '#264653'; // Dark text
    }
  }
